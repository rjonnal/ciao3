...
class Zernike:
    ...
    def zeqn(self,n,m,kind='h',forceRecompute=False):
        ...
        srange = list(range((n-absm)//2+1))

        cdict = {}

        for s in srange:
            jrange = list(range(((n-absm)//2)-s+1))
            for j in jrange:

                # Subtract 1 from absm to determine range,
                # only when m<0.
                if m<0:
                    krange = list(range((absm-1)//2+1))
                else:
                    krange = list(range(absm//2+1))
        ...
        cdict = {key: value for key, value in list(cdict.items()) if value}
    ...
    def convert_to_surface(self,params,X,Y,mask=None):
        ...
        for item in list(params[1].items()):
